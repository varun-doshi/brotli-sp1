...
fn create_proof_fixture(
    ...
    let decoded_batch_data = <Array<MessageWithMetadataStruct>>::abi_decode(bytes).unwrap();

    // Create the testing fixture so we can test things end-to-end.
    let fixture = SP1BrotliProofFixture {
        block_number: decoded_batch_data
            .iter()
            .map(|m| m.message.header.block_number as u32)
            .collect(),
